...
test('clicking "Add a new event" navigates to search page with only name and search button', async ({ page }) => {
  await page.goto(BASE_URLS.SIGN_IN)
  await submitSignInForm(page, TEST_USERS.KNOWN_USER)
...
test('searching Wikipedia for a known term redirects to new-event page with pre-filled fields', async ({ page }) => {
  await page.goto(BASE_URLS.SIGN_IN)
  await submitSignInForm(page, TEST_USERS.KNOWN_USER)
...
test('searching Wikipedia for gibberish shows nothing found error on search page', async ({ page }) => {
  await page.goto(`${BASE_URLS.HOME}/ui/search`)
  await page.waitForSelector('[data-testid="name-input"]')
...
test('pressing Enter in name field triggers Wikipedia search', async ({ page }) => {
  await page.goto(BASE_URLS.SIGN_IN)
  await submitSignInForm(page, TEST_USERS.KNOWN_USER)
...
test('name and reference URL are read-only on new-event page', async ({ page }) => {
  await page.goto(BASE_URLS.SIGN_IN)
  await submitSignInForm(page, TEST_USERS.KNOWN_USER)
...
test('"Search again" button navigates back to search page with empty fields', async ({ page }) => {
  await page.goto(BASE_URLS.SIGN_IN)
  await submitSignInForm(page, TEST_USERS.KNOWN_USER)
  ...
  await page.waitForSelector('[data-testid="search-again-action"]', { timeout: 15000 })
...
test('Wikipedia Text Preview and Related Links sections are scrollable', async ({ page }) => {
  await page.goto(BASE_URLS.SIGN_IN)
  await submitSignInForm(page, TEST_USERS.KNOWN_USER)
  ...
  await page.waitForSelector('[data-testid="wiki-text-preview"]', { timeout: 15000 })

  const textPreviewOverflow = await page.getByTestId('wiki-text-preview').evaluate(
    (el) => getComputedStyle(el).overflowY
  )
  expect(textPreviewOverflow).toBe('auto')

  const linksOverflow = await page.getByTestId('wiki-links-list').evaluate(
    (el) => getComputedStyle(el).overflowY
  )
  expect(linksOverflow).toBe('auto')
})

test('"Search again" button text says "Search again" not "Search Wikipedia"', async ({ page }) => {
  await page.goto(BASE_URLS.SIGN_IN)
  await submitSignInForm(page, TEST_USERS.KNOWN_USER)
...
test('navigating directly to /ui/new-event without search redirects to search page', async ({ page }) => {
  await page.goto(BASE_URLS.SIGN_IN)
  await submitSignInForm(page, TEST_USERS.KNOWN_USER)
