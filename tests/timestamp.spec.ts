...
describe('timestampToDateComponents', () => {
  ...
  it('should handle leap year correctly for 4 AD', () => {
    ...
    assert.strictEqual(mar1Year4 - jan1Year4, 31 + 29)
  })

  it('should handle non-leap century year 100 AD', () => {
    const jan1Year100 = dateComponentsToTimestamp({ year: 100, month: 1, day: 1 })
    const mar1Year100 = dateComponentsToTimestamp({ year: 100, month: 3, day: 1 })
    assert.strictEqual(mar1Year100 - jan1Year100, 31 + 28)
  })

  it('should handle leap century year 400 AD', () => {
    const jan1Year400 = dateComponentsToTimestamp({ year: 400, month: 1, day: 1 })
    const mar1Year400 = dateComponentsToTimestamp({ year: 400, month: 3, day: 1 })
    assert.strictEqual(mar1Year400 - jan1Year400, 31 + 29)
  })
})
...
describe('fromTimestamp', () => {
  ...
  it('should format large BC year with commas', () => {
    ...
    assert.strictEqual(result, '40,000 BC')
  })
  ...
  it('should format modern date correctly', () => {
    ...
    assert.strictEqual(result, 'June 19, 2025 AD')
  })
})
...
describe('toTimestamp', () => {
  it('should parse year-only BC date', () => {
    ...
    assert.strictEqual(components.year, -39999)
    assert.strictEqual(components.month, 1)
    assert.strictEqual(components.day, 1)
  })

  it('should parse year-only AD date', () => {
    const result = toTimestamp('2025 AD')
    const components = timestampToDateComponents(result)
    assert.strictEqual(components.year, 2025)
    assert.strictEqual(components.month, 1)
    assert.strictEqual(components.day, 1)
  })
  ...
  it('should parse month and year', () => {
    ...
    const components = timestampToDateComponents(result)
    assert.strictEqual(components.year, 2025)
    assert.strictEqual(components.month, 6)
    assert.strictEqual(components.day, 1)
  })
  ...
  it('should parse full date', () => {
    ...
    assert.strictEqual(components.year, 2025)
    assert.strictEqual(components.month, 6)
    assert.strictEqual(components.day, 19)
  })

  it('should parse BCE style', () => {
    const result = toTimestamp('306 BCE')
    const components = timestampToDateComponents(result)
    assert.strictEqual(components.year, -305)
  })
  ...
  it('should throw for missing era suffix', () => {
    assert.throws(() => toTimestamp('2025'), /missing era suffix/)
  })

  it('should throw for invalid day', () => {
    assert.throws(() => toTimestamp('February 30, 2025 AD'), /Invalid day/)
  })

  it('should accept Feb 29 in leap year', () => {
    const result = toTimestamp('February 29, 2024 AD')
    const components = timestampToDateComponents(result)
    assert.strictEqual(components.month, 2)
    assert.strictEqual(components.day, 29)
  })

  it('should reject Feb 29 in non-leap year', () => {
    assert.throws(() => toTimestamp('February 29, 2025 AD'), /Invalid day/)
  })
})
